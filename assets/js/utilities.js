...
function emotion(event) {
  ...
  const { index, emotion: emotionString, id } = elem.dataset;

  const emotion = emotionString === "true" ? true : false;

  console.log(index, emotion, id);

  const query = getLS("query")[index];

  const userExistIndex = checkUser();
  console.log(userExistIndex);

  console.log("current index", index);
  if (userExistIndex === -1) {
    addEmotion(id, emotion);
  } else if (userExistIndex >= 0) {
    console.log(userExistIndex);
...
function addEmotion(queryId, emotion){
    
    const boolean = getLS("boolean");
    const { email } = getLS("loggedInUser");
